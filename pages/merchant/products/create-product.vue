    ...
    >
      <div class="relative p-4 mt-3 w-full max-w-4xl h-full md:h-auto">
        ...
        <div class="relative bg-white rounded-lg shadow">
          ...
          <!-- Modal body start-->
          <div class="px-6 pb-6">
            <ValidationObserver v-slot="{ handleSubmit }" ref="filterForm">
              <form @submit.prevent="handleSubmit(saveFilter)">
                <!-- form row start -->
                <div class="flex items-center gap-x-4">
                  <!-- single input start -->
                  <div class="col-span-2">
                    <label for="" class="text-gray-600 mb-2 block">Filter name</label>
                    <ValidationProvider v-slot="{ errors }" rules="required">
                      <input
                        ...
                        :class="[errors[0] ? 'border-red-500' : 'border-pgray-300']"
                  ...
                  <!-- single input start -->
                  <div>
                    <label for="" class="text-gray-600 mb-2 block">Price</label>
                    <ValidationProvider v-slot="{ errors }" rules="required">
                      <vue-numeric
                        class="block w-full border text-right border-gray-300 px-4 py-2 text-gray-600 rounded placeholder-gray-400 focus:border-primary focus:border-2 focus:ring-0"
                        id="purchase_price"
                        seperator=","
                        :precision="2"
                        currency="Rs."
                        v-model="filter.filter_price"
                      ></vue-numeric>
                  ...
                  <!-- filter image preview start -->
                  <img
                    :src="filter_image_prev"
                    alt=""
                    class="mt-6 w-10 h-10 rounded-full"
                  />
                  <!-- filter image preview end -->

                  <!-- filter status check box start -->
                  <div class="flex mt-6">
                  ...
                  <!-- filter status check box end -->
                  <!-- filter save buttons start -->
                  <div class="flex mt-6">
                    <div class="flex items-center gap-x-2">
                      <button
                        v-if="!filterEditMode"
                        class="bg-green-500 px-3 py-1 text-sm text-white rounded-md"
                      >
                        Save
                      </button>
                      <button
                      ...
                      <button
                        class="bg-gray-500 px-3 py-1 text-sm text-white rounded-md"
                        type="button"
                ...
                <div class="mt-5">
                  ...
                  <div class="flex gap-x-5 m-5" v-else>
                    <div class="flex justify-between gap-7">
                      ...
                      >
                        <img
                          v-if="image.source == 'db'"
                          :src="image.image_path"
                          class="cursor-pointer w-28 transition"
                          alt=""
                          for="product_image"
                          @click="setFilterImage(index)"
                        />
                      </div>
                    </div>
                  </div>
                ...
                <table class="w-full text-sm text-left text-gray-500 mt-5">
                  ...
                  <tbody>
                    <tr v-if="added_filters.length == 0">
                      <td colspan="6" class="text-center">No filters found!</td>
                    </tr>
                    <tr
                      v-else
                      class="bg-white border-b hover:bg-gray-50"
                      v-for="(filter, index) in added_filters"
                      :key="index"
                    >
                      ...
                      <td class="px-6 py-4">{{ filter.filter_price | currency }}</td>
                      <td class="px-6 py-4">
                        <img
                          :src="getFilterImageFortheTable(filter.filter_image)"
                          alt=""
                      ...
                      <td class="px-6 py-4 text-right">
                        <button
                          ...
                          @click="editFilterData(filter)"
                        >
                          Edit
                        </button>
                        <button
                          type="button"
                          @click="deleteFilterData(filter)"
      ...
      <ValidationObserver v-slot="{ handleSubmit }" ref="form">
        <form @submit.prevent="handleSubmit(saveProduct)">
          ...
          <div class="grid grid-rows-1 md:grid-rows-2 grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <!-- single input end -->
            <!-- single input start -->
            <div v-if="$auth.user.company.manual_prod_code">
