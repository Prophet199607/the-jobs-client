      ...
      <form @submit.prevent="handleSubmit(saveOrder)">
        <div class="grid grid-rows-2 grid-cols-12 gap-6 mb-10">
          ...
          <div class="col-span-12 md:col-span-4 shadow-lg p-4 rounded">
            ...
            <div class="space-y-2">
              <div v-for="product in product.products" :key="product.product_id">
            ...
            >
              <p>Subtotal</p>
              <p>{{ !product.total_amount ? 0 : product.total_amount | currency }}</p>
            </div>
            <div
            ...
            <div v-else class="text-sm text-red-500 font-medium">
              No delivery available for {{ delivery_address.city_name }}. You can collect
              the package from our nearest warehouse.
            </div>
              ...
              <p>
                {{
                  !product.total_amount
                    ? 0
                    : (product.total_amount + delivery_charge) | currency
                }}
              </p>
            </div>
            <div class="mt-10">
            ...
            <div class="mt-5 text-center mb-8">
              <button
                v-if="$auth.loggedIn && !noDeliveryAvailable"
                type="submit"
              ...
              <button
                v-else
                class="w-full block px-4 py-3 text-center text-sm text-white cursor-not-allowed bg-red-400 border-red-400 rounded transition uppercase font-roboto font-medium"
                disabled
              >
                {{ form.payment_method == 1 ? "Pay Now" : "Place the order" }}
              </button>
...
export default {
  ...
  async created() {
    if (this.product && this.$route.query.p_id) {
      this.loadProductDetails(
        this.$route.query.p_id,
        this.$route.query.q,
        this.$route.query.f
      );
    }
  ...
  computed: {
    product() {
      return this.$store.getters["cart/getbuyNowProductData"];
    },
  },

  methods: {
    loadProductDetails(product_code, qty, filter) {
      this.$store.dispatch("products/loadBuyNowProductDetails", {
        product_code,
        qty,
        filter,
      });
