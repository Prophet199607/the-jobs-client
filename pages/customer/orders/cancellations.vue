<template>
  <div>
    ...
    <div class="container py-4 flex items-center gap-3 px-6 text-sm md:text-base">
      ...
      <p class="text-gray-600 font-medium">My Cancellations</p>
    </div>
    <!-- breadcrumbs end -->
    <div
      class="text-gray-500 text-sm text-center my-52"
      v-if="
        cancelled_orders && cancelled_orders.data && cancelled_orders.data.length == 0
      "
    >
    ...
    <div
      ...
      v-for="order in cancelled_orders.data"
      :key="order.id"
    ...
    <div class="my-3 float-right">
      <pagination
        ...
        :limit="3"
        @pagination-change-page="loadCustomerCancelledOrders"
      ></pagination>
    </div>
  </div>
</template>
...
export default {
  computed: {
    cancelled_orders() {
      return this.$store.getters["order/getCustomerCancelledOrders"];
    },
  },

  mounted() {
    this.loadCustomerCancelledOrders();
  },

  methods: {
    loadCustomerCancelledOrders(page = 1) {
      this.$store.dispatch("order/loadCustomerCancelledOrders", page);
    },
    ...
    navigateToPage(path, order_code) {
      this.$router.push({ path, query: { no: order_code, c: true } });
    },
  },
};
</script>
